...
task_set_clipboard(void *userdata) {
    ...
    char *text = userdata;
    const char *image_prefix = "content://local.foldlink.clipboard/images/";
    if (!strncmp(text, image_prefix, strlen(image_prefix))) {
        free(text); // Preserve the original Mac image instead of echoing its Android URI.
        return;
    }

...
sc_input_manager_process_key(struct sc_input_manager *im,
    ...
    if (event->scancode == SDL_SCANCODE_CAPSLOCK) {
        if (im->kp && im->kp->hid && event->type == SDL_EVENT_KEY_DOWN && !event->repeat)
            sc_foldlink_language_request();
        return;
    }
    SDL_Keycode sdl_keycode = event->key;
    // macOS Command is not Android's Meta/launcher key. Never leave a remote
    // Meta key pressed while handling a local clipboard shortcut.
    if (event->scancode == SDL_SCANCODE_LGUI || event->scancode == SDL_SCANCODE_RGUI) return;
    SDL_Keycode clipboard_key = event->scancode == SDL_SCANCODE_C ? SDLK_C
                              : event->scancode == SDL_SCANCODE_V ? SDLK_V
                              : event->scancode == SDL_SCANCODE_X ? SDLK_X : SDLK_UNKNOWN;
    // FoldLink: normalize both Mac Command and Control clipboard shortcuts.
    bool clipboard_mod = (event->mod & (SDL_KMOD_CTRL | SDL_KMOD_GUI))
                      && !(event->mod & (SDL_KMOD_ALT | SDL_KMOD_SHIFT));
    if (clipboard_mod && clipboard_key != SDLK_UNKNOWN) {
        if (control && im->kp && !paused && !disconnected
                && event->type == SDL_EVENT_KEY_DOWN && !event->repeat) {
            if (clipboard_key == SDLK_V) {
                if (!sc_foldlink_image_paste()) set_device_clipboard(im, true, SC_SEQUENCE_INVALID);
            } else {
                get_device_clipboard(im, clipboard_key == SDLK_C ? SC_COPY_KEY_COPY : SC_COPY_KEY_CUT);
            }
        }
        return;
    }
    if (event->mod & SDL_KMOD_GUI) return;
    ...
    if (keycode == SC_KEYCODE_UNKNOWN && !im->kp->hid) {
        return;
    }
...
sc_input_manager_handle_event(struct sc_input_manager *im,
    ...
    switch (event->type) {
        case SC_EVENT_FOLDLINK_LANGUAGE:
            if (im->controller && im->kp && !im->disconnected) {
                send_keycode(im, AKEYCODE_LANGUAGE_SWITCH, SC_ACTION_DOWN, "LANGUAGE_SWITCH");
                send_keycode(im, AKEYCODE_LANGUAGE_SWITCH, SC_ACTION_UP, "LANGUAGE_SWITCH");
                LOGI("FoldLink: language switch sent");
            }
            break;

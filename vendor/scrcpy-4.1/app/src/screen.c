...
resize_for_content(struct sc_screen *screen, struct sc_size old_content_size,
    ...
    if (!screen->flex_display) {
        struct sc_size window_size = screen->fold_animating
                                   ? screen->fold_to
                                   : sc_sdl_get_window_size(screen->window);
        // Preserve the intended scale when a fold interrupts an animation.
    ...
    assert(is_windowed(screen));
    if (!screen->flex_display && !sc_foldlink_reduce_motion()) {
        screen->fold_from = sc_sdl_get_window_size(screen->window);
        screen->fold_to = target_size;
        screen->fold_started = SDL_GetTicks();
        screen->fold_animating = true;
        // Temporarily release the target ratio constraint so width can interpolate.
        SDL_SetWindowAspectRatio(screen->window, 0, 0);
        LOGI("FoldLink: animated resize to %ux%u", target_size.width, target_size.height);
    } else {
...
void
sc_screen_fold_tick(struct sc_screen *screen) {
    if (!screen->fold_animating) return;
    if (!is_windowed(screen) || sc_foldlink_reduce_motion()) {
        screen->fold_animating = false;
        set_aspect_ratio(screen, screen->content_size);
        if (is_windowed(screen)) sc_sdl_set_window_size(screen->window, screen->fold_to);
        return;
    }
    double t = (SDL_GetTicks() - screen->fold_started) / 360.0;
    double e = sc_foldlink_ease(t);
    struct sc_size size = {
        .width = screen->fold_from.width + (screen->fold_to.width - (double)screen->fold_from.width) * e,
        .height = screen->fold_from.height + (screen->fold_to.height - (double)screen->fold_from.height) * e,
    };
    sc_sdl_set_window_size(screen->window, size);
    if (t >= 1) {
        screen->fold_animating = false;
        set_aspect_ratio(screen, screen->content_size);
    }
